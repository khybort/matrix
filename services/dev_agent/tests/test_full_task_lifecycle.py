...
async def test_lifecycle_happy_path(pg_pool, tmp_path):
    """pending → running → awaiting_review with events recorded."""
    ...
    row = await pg_pool.fetchrow("SELECT status, started_at, finished_at FROM dev_tasks WHERE id=$1", task_id)
